...
namespace Grand.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        public virtual async Task<IList<Order>> GetAllOrders(string storeId, OrderStatus? os, PaymentStatus? ps, ShippingStatus? ss, DateTime? startTimeUtc, DateTime? endTimeUtc)
        {
            int? orderStatusId = null;
            if (os.HasValue)
                orderStatusId = (int)os.Value;
            ...
            var report = (from oq in query where oq.CheckoutAttributeDescription.StartsWith("Port Of Delivery") select oq);
